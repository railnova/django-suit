...
class Menu(object):
    ...
    def make_menu(self, config):
        ...
        if not isinstance(config, (tuple, list)):
            raise TypeError(
                "Django Suit MENU config parameter must be "
    ...
    def make_app(self, app_def):
        if isinstance(app_def, dict):
            ...
            raise TypeError(
                "MENU list item must be string or dict. Got %s" % repr(app_def)
            )
        if app:
    ...
    def make_model(self, model_def, app_name):
        if isinstance(model_def, dict):
            ...
            raise TypeError(
                "MENU list item must be string or dict. Got %s" % repr(model_def)
            )
        if model:
    ...
    def model_is_forbidden(self, model):
        return model["permissions"] and not self.user_has_permission(
            model["permissions"]
        )
    ...
    def activate_menu(self, menu):
        for app in menu:
            ...
            if not self.app_activated and (
                self.request.path == app["url"]
