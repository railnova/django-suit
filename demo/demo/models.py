...
class Country(models.Model):
    ...
    code = models.CharField(
        max_length=2, help_text="ISO 3166-1 alpha-2 - two character country code"
    )
    independence_day = models.DateField(blank=True, null=True)
    area = models.BigIntegerField(blank=True, null=True)
    population = models.BigIntegerField(blank=True, null=True)
    description = models.TextField(
        blank=True, help_text="Try and enter few some more lines"
    )
...
class Showcase(models.Model):
    ...
    multiple_in_row = models.CharField(
        max_length=64, help_text="Help text for multiple"
    )
    ...
    boolean_with_help = models.BooleanField(
        default=False, help_text="Boolean field with help text"
    )
    ...
    country2 = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="showcase_country2_set",
        verbose_name="Django Select 2",
        on_delete=models.SET_NULL,
    )
    raw_id_field = models.ForeignKey(
        Country,
        null=True,
        blank=True,
        related_name="showcase_raw_set",
        on_delete=models.SET_NULL,
    )
